...
In the interest of full transparency and per the instructions of staff: we used ChatGPT to review and edit our code for clarity and efficieny, as well as to provide annotation.
...
Define the function and its 2 inputs (month and year).
...
Then, using the scrapping/crawling code above as a guide...
    Use BeautifulSoup to parse the HTML content of the page. This code to be based on the code above.

Begin the For loop. For each action find:
  Title and Link
  Date
  Category
  Agency Information:
    This is also where we create the conditions to exit the While loop. If the date is older than the inputs of the function, the function should just return what its got and stop looping.
...
```{python}
# Load the data
file_path = ('/Users/griffinsharps/Documents/Repos/Kajie-Griffin-ps5/enforcement_actions_2021_01.csv') 
...
```{python}
# Load the dataset
data = pd.read_csv('/Users/griffinsharps/Documents/Repos/Kajie-Griffin-ps5/enforcement_actions_2021_01.csv')
...
# Plot the ratio of enforcement actions per population
fig, ax = plt.subplots(1, 1, figsize=(20, 12))  # Larger figure size
district_map.plot(
    column='Enforcement_Ratio', cmap='OrRd', linewidth=0.5, ax=ax, edgecolor='0.5',
    legend=True, legend_kwds={'shrink': 0.4, 'label': "Enforcement Actions per Capita", 'orientation': "horizontal", 'pad': 0.05}
)

# Customize the title and layout for a clearer display
ax.set_title("Ratio of Enforcement Actions to Population by U.S. Attorney District", fontsize=18, pad=20)
ax.set_axis_off()

# Tighten spacing around the map
