...
df = pd.read_csv('/Users/Kaijie/Desktop/Kajie-Griffin-ps5/enforcement_actions_2021_01.csv')

# Categorize the data
def categorize_group(row):
    if "state" in row['Agency'].lower():
        return 'State Enforcement Agencies'
    else:
        return 'Criminal and Civil Actions'

df['Category'] = df.apply(categorize_group, axis=1)

# Convert date if necessary
df['Date'] = pd.to_datetime(df['Date'])

# Group and count actions per category per time period (e.g., monthly)
action_counts = df.groupby([pd.Grouper(key='Date', freq='M'), 'Category']).size().reset_index(name='Counts')

# Create a line chart
chart = alt.Chart(action_counts).mark_line(point=True).encode(
    x=alt.X('month(Date):T', title='Month'),
    y=alt.Y('Counts:Q', title='Number of Enforcement Actions'),
    color=alt.Color('Category:N', legend=alt.Legend(title="Category")),
    tooltip=[alt.Tooltip('month(Date):T', title='Month'), 'Counts', 'Category']
).properties(
...
topic_counts.columns = ['Topic', 'Number of Actions']

# Create a line chart using Altair
line_chart = alt.Chart(topic_counts).mark_line(point=True).encode(
    x=alt.X('Topic', sort=None, title='Topic'),
    y=alt.Y('Number of Actions', title='Number of Actions'),
    tooltip=['Topic', 'Number of Actions']
).properties(
    title='Number of Enforcement Actions by Topic',
    width=600,
    height=400
)
